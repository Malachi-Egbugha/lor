...
function createServer() {
    ...
    app.get('/getcustomer/:accountnumber', routeprotection_1.apiauthorize, (req, res) => __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            const redisdata = yield redisClient.get(accountnumber);
            if (redisdata) {
                res.json({ queryresult: JSON.parse(redisdata), status: true });
            }
            else {
                let response = yield (0, axios_1.default)(config);
                yield redisClient.setEx(accountnumber, 259200, JSON.stringify(response.data));
